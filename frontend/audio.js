...
(function (window) {
  ...
  const SoundFX = {
    ...
    /**
     * 6. БИО-ОТКЛИК ТЕРРАРИУМА / СИМБИОНТА (Импульсный резонанс)
     */
    ...
    /**
     * 8. КИБЕР-КОШКА CAT181b: МУРЛЫКАНЬЕ (PURR)
     * Низкочастотная модулированная вибрация ~65-75 Гц
     */
    playCatPurr: function () {
      if (!isSoundEnabled) return;
      const ctx = getAudioContext();
      if (!ctx) return;
      ...
      // Частотная модуляция вибрации горла (22 Гц амплитудная дрожь)
      mod.type = 'sine';
    ...
    /**
     * 9. КИБЕР-КОШКА CAT181b: МЯУ / ПИЛИКАНЬЕ ЧИПА (MEOW / CHIRP)
     * Гармонический кибернетический перелив с восходящим глиссандо
     */
    playCatMeow: function () {
      if (!isSoundEnabled) return;
      const ctx = getAudioContext();
      if (!ctx) return;
    ...
    /**
     * 10. КИБЕР-КОШКА CAT181b: ХРУСТ ПРИ ЕДЕ (CRUNCH / FEED)
     */
    playCatCrunch: function () {
      if (!isSoundEnabled) return;
      const ctx = getAudioContext();
      if (!ctx) return;
      ...
      [0, 0.08, 0.16].forEach((delay, idx) => {
        ...
        filter.frequency.setValueAtTime(1400 + idx * 300, t + delay);
        ...
        noise.start(t + delay);
        noise.stop(t + delay + 0.045);
      });
    },

    /**
     * 11. КИБЕР-КОШКА CAT181b: ИГРА / КОЛОКОЛЬЧИК (PLAY)
     */
    playCatPlay: function () {
      if (!isSoundEnabled) return;
      const ctx = getAudioContext();
      if (!ctx) return;
